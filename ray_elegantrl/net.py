...
class CriticAdv_Multi(nn.Module):
    def __init__(self, state_dim, mid_dim, reward_dim, if_use_dn=False):
        super().__init__()
        ...
        [layer_norm(self.tails[-1], std=0.5) for _ in range(reward_dim)]  # output layer for Q value

    def forward(self, state):
        tmp = self.net(state)
        return torch.cat([self.tails[i](tmp) for i in range(len(self.tails))], dim=1)  # Multi Q value
