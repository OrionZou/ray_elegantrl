...
class InterActor(object):
    ...
    def exploit_env(self, policy, eval_times):
    ...
    while (evaluator.record_totalstep < evaluator.break_step) or (evaluator.record_satisfy_reward):
        ...
        if evaluator.pre_eval_times > 0:
            eval_results = ray.get(
                [interactors[i].exploit_env.remote(policy_id, eval_times=evaluator.pre_eval_times) for i in
                 range(rollout_num)])
        ...
        if if_eval:
            eval_results = ray.get(
                [interactors[i].exploit_env.remote(policy_id, eval_times=(evaluator.eval_times))
