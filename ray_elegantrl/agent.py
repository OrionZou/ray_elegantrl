...
from ray_elegantrl.net import Critic, CriticAdv, CriticTwin, CriticAdv_Multi
...
class AgentDoubleDQN(AgentDQN):
    ...
    @staticmethod
    def select_action(state, policy) -> int:  # for discrete action space
        explore_rate = 0.25
...
class AgentMPO(AgentBase):
    ...
    def get_obj_critic_raw(self, buffer, batch_size):
        with torch.no_grad():
            ...
            ex_next_q = ex_next_q.reshape(batch_size, self._num_samples)
            next_q = ex_next_q.mean(dim=1).unsqueeze(dim=1)
            q_label = reward + mask * next_q
